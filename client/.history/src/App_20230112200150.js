...
const App = () => {
  ...
  // Keeps track on whether someone has won or not
  const [win, setWin] = useState(false)
  var winClass = win ? 'win-screen' : 'win-screen hide'
  ...
  const checkWin = () => {
    ...
    else if (cards.filter(c => c.team === 'blue' && !c.clicked).length === 0) {
      newWin = true
    }
    else if (cards.filter(c => c.team === 'blue' && !c.clicked).length === 0) {
      newWin = true
  ...
  return (
    <div className="App">
      ...
      <div className='wrapper'>
        ...
        <div className={winClass} id='win-screen'></div>
      </div>
