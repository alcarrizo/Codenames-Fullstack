...
io.on('connection', socket => {
    ...
    socket.on('give-clue', (clue) => {
        var team = Game.players.filter(p => p.id === socket.id).team
        Game.clue = clue.word
        Game.clueNum = clue.num === '10' ? "&infin;" : clue.num
        clueGiven = true
        cardsPicked = 0
        pickLimit = clue.num === '10' || clue.num === '0' ? 10 : Number(clue.num)

        io.sockets.emit('clue-given', { team: team, word: Game.clue, limit: Game.pickLimit, num: Game.clueNum })
    })
